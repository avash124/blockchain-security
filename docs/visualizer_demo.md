# Visualizer Demo — Euler Finance Exploit
...
graph TD
    ...
    V2["🚨 State written before balance/invariant check (checks-effects violated)"]
    F2["🛡 Apply checks-effects-interactions: validate invariants before state mutation"]
    V3["🚨 Health / collateral factor not re-validated after flash-funded operations"]
    F3["🛡 Re-check health factor after every balance-altering call inside the loan"]
